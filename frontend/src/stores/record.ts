...
export const useRecordStore = defineStore('record', () => {
  ...
  const evaluationTasks = ref<Record<number, EvaluationTask>>({})
  ...
  async function fetchEvaluation(recordId: number) {
    const known = evaluationTasks.value[recordId]
    const task = known
      ? await getEvaluationApi(known.taskId)
      : await getLatestEvaluationApi(recordId)
    evaluationTasks.value[recordId] = task
    return task
  }
