      ...
      <div class="report-body" id="report-print">
        ...
        <div class="score-banner" :class="evaluating ? 'is-evaluating' : ''">
          <template v-if="evaluating">
            ...
            <div>
              <div class="banner-label">{{ evaluationStatus.title }}</div>
              <div class="banner-sub">{{ evaluationStatus.detail }}</div>
            </div>
            <div v-if="evaluationTask" class="task-trace">
              <span>任务 #{{ evaluationTask.taskId }}</span>
              <span>重试 {{ evaluationTask.retryCount }} / {{ evaluationTask.maxRetries }}</span>
            </div>
          </template>
          <template v-else>
...
const evaluationStatus = computed(() => {
  const task = evaluationTask.value
  if (!task || task.status === 'pending') {
    return { title: '评测已进入队列', detail: '答案已安全保存，正在等待 AI 评测资源。' }
  }
  if (task.status === 'running') {
    return { title: 'AI 正在分析回答', detail: '正在生成逐题反馈与能力维度评分。' }
  }
  if (task.status === 'failed') {
    const exhausted = task.retryCount >= task.maxRetries
    return exhausted
      ? { title: '评测需要处理', detail: task.errorMessage || '自动重试已结束，请联系管理员重新提交。' }
      : { title: '评测正在自动恢复', detail: `本次执行未完成，系统将自动重试（${task.retryCount}/${task.maxRetries}）。` }
  }
  return { title: '评测完成', detail: '正在刷新报告。' }
})
...
function startPolling() {
  ...
  pollTimer = setInterval(async () => {
    ticks++
    try {
      const task = await recordStore.fetchEvaluation(recordId)
      if (task.status === 'success') {
        await recordStore.fetchRecords()
      }
    } catch { /* 记录轮询仍可作为兼容回退 */ }
    if (ticks % 5 === 0) await recordStore.fetchRecords()
    const exhausted = evaluationTask.value?.status === 'failed'
      && evaluationTask.value.retryCount >= evaluationTask.value.maxRetries
    if (!evaluating.value || exhausted || ticks >= 45) {
...
onMounted(async () => {
  await recordStore.fetchRecords()
  if (evaluating.value) {
    try { await recordStore.fetchEvaluation(recordId) } catch { /* 兼容旧记录 */ }
    startPolling()
  }
})
...
.score-banner {
  ...
  &.is-evaluating {
    ...
    gap: 12px;
    background: linear-gradient(135deg, #334c68, #1d2e45);
  }
