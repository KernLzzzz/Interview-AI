...
export function voToQuestion(vo: QuestionVO): Question {
  return {
    ...
    expectedAnswer: 'expectedAnswer' in vo ? (vo as AdminQuestionVO).expectedAnswer : undefined,
    keywords: 'keywords' in vo ? (vo as AdminQuestionVO).keywords : undefined
  }
}
...
/** 管理端编辑详情；答案字段仅通过这个受管理员权限保护的接口返回。 */
export function getAdminQuestionApi(id: number): Promise<AdminQuestionVO> {
  return request<AdminQuestionVO>({ url: `/questions/admin/${id}`, method: 'GET' })
}
